...
void dampResonators(double damping) 
{
    for(int i=0;i<nf;i++) {
        yt_1[i] *= damping;
	yt_2[i] *= damping;
...
*/
double 
computeSoundBuffer(float *output, float *force, int nsamples, bool doPlot, int numResonators) 
{
...
printf("argc:%d optind:%d\n", argc, optind);
	...
	if(car != -1) {
		switch(car) {
			case ' ': dampResonators(0.3); break;

			case '0': numResonators = nf;  break;

			case 'r': mode = resonators;   break;
			case 'f': mode = tuneFreq; printf("tuneFreq\n");    break;
			case 'd': mode = tuneDamp; printf("tuneDamp\n");    break;
			case 'a': mode = tuneAmp;  printf("tuneAmp\n");     break;
			...
			default:
				...
				double delta = 0.1 * (car == '-')?-1.0:1.0;
			    	...
			    	// update params
				switch(mode) {
					case tuneFreq: f[num] += delta; break;
					case tuneDamp: d[num] += delta; break;
					case tuneAmp:  a[num] += delta; break;
				}
	...
	total = computeSoundBuffer(buffer, cosForce, bufferSize, doPlot, numResonators);
